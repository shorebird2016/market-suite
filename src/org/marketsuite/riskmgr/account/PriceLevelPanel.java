...
class PriceLevelPanel extends AbstractGraphPanel {
    PriceLevelPanel() {
...
//        });
    ...
    public void drawLevels(StopLevelInfo sli) {//ArrayList<StopLevel> stop_levels) {
        //calculate full range y value, assume stop levels are sorted
        ArrayList<StopLevel> stop_levels = sli.getStopLevels();
        ...
        for (StopLevel lvl : stop_levels) {
            ...
            if (StopLevelInfo.isAtrMethod(id)) {//ATR: pink
                c = Color.magenta;
                end_idx = length / 4;
                stroke_width = 0.5f;
            }
            else if (StopLevelInfo.isSwpMethod(id)) {//SWP: green
                c = Color.green.darker();
                end_idx = length / 2;
                stroke_width = 0.8f;
            }
            else if (StopLevelInfo.isPercentMethod(id)) {//Percent: blue
            ...
            else if (StopLevelInfo.isCurrentStop(id)) {//Current Stop: solid red
                c = Color.red.darker();
                end_idx = length / 10;
                stroke_width = 1.7f;
            }
            else if (StopLevelInfo.isSmaMethod(id)) {//SMA stop
                c = Color.green.darker();
                end_idx = length / 2;
                stroke_width = 0.8f;
            }
            ...
            data_item = ts.getDataItem(end_idx);//use end_idx to find coordinate to draw, end = left side
            period = data_item.getPeriod();
            xlvl_left = period.getLastMillisecond();//period.getFirstMillisecond();TODO no difference
            levels[index] = new XYLineAnnotation(xlvl_left, lvl.getLevel(), x_begin, lvl.getLevel(),
                new BasicStroke(stroke_width), c);

            //don't draw custom level
            ...
            data_item = ts.getDataItem((int)(end_idx * 0.95));
            long xlbl_left = data_item.getPeriod().getLastMillisecond();
            float lvl_y = (float)(lvl.getLevel()/* - range / 150*/);
            labels[index] = new XYTextAnnotation(id + " (" + FrameworkConstants.DOLLAR_FORMAT.format(lvl_y) + ")", xlbl_left, lvl_y);
