...
import org.marketsuite.framework.model.type.DatapadMode;
//TODO: move to component package after redo these:
//TODO:    (1)circular reference to view class
//TODO:    (2)general purpose datapad instead of just serving AnalysisGraphFrame
//TODO:    (3)use component.bundle
...
public class ChartLayerUI<V extends javax.swing.JComponent> extends AbstractLayerUI<V> {
    ...
    private final List<TraceInfo> indicatorTraceInfos = new ArrayList<>();
    ...
    private final AbstractGraphPanel _pnlGraph;
    private static final List<String> params = Collections.unmodifiableList(
        Arrays.asList(
            Constants.COMPONENT_BUNDLE.getString("StockHistory_Open"),
            Constants.COMPONENT_BUNDLE.getString("StockHistory_High"),
            Constants.COMPONENT_BUNDLE.getString("StockHistory_Low"),
            Constants.COMPONENT_BUNDLE.getString("StockHistory_Close"),
            Constants.COMPONENT_BUNDLE.getString("StockHistory_Volume")
        ));
    ...
    public ChartLayerUI(AbstractGraphPanel container) {
        _pnlGraph = container;
    }

    //drawing the yellow rectangle representing data items at cursor dot
    private void drawDataPad(Graphics2D g2) {
        if (_DatapadMode.equals(DatapadMode.HIDE))//redraw clears automatically
            return;

        //save previous font, calc various font info
        final Font old_font = g2.getFont();
        final Font param_font = old_font;
        final FontMetrics param_font_metrics = g2.getFontMetrics(param_font);
        final Font value_font = old_font.deriveFont(old_font.getStyle() | Font.BOLD, (float)old_font.getSize() + 1);
        final FontMetrics value_font_metrics = g2.getFontMetrics(value_font);
        final Font date_font = old_font.deriveFont((float) old_font.getSize() - 1);
        final FontMetrics date_font_metrics = g2.getFontMetrics(date_font);

        //get data from panel that are already calculated
        final List<ChartData> chartDatas = _pnlGraph.getChartDatas();
        List<String> values = new ArrayList<>();
        final ChartData chartData = chartDatas.get(mainTraceInfo.getDataIndex());
        ...
        final DecimalFormat integerFormat = new DecimalFormat("###,###");//TODO switch to Framework constants

        // It is common to use OHLC for chat, instead of using PrevPrice.
        values.add(stockPriceDecimalFormat(chartData.openPrice));
        values.add(stockPriceDecimalFormat(chartData.highPrice));
        values.add(stockPriceDecimalFormat(chartData.lowPrice));
        values.add(stockPriceDecimalFormat(chartData.lastPrice));
        values.add(integerFormat.format(chartData.volume));
        final List<String> indicatorParams = new ArrayList<>();
        final List<String> indicatorValues = new ArrayList<>();
        ...
        // param_font_metrics will always "smaller" than value_font_metrics.
        int totalInfoHeight = Math.max(param_font_metrics.getHeight(),
            value_font_metrics.getHeight()) * values.size() + paramValueHeightMargin * (values.size() - 1);
        for (String param : params) {
            final String value = values.get(index++);
            final int paramStringWidth = param_font_metrics.stringWidth(param + ":")
                + paramValueWidthMargin + value_font_metrics.stringWidth(value);
        ...
        if (indicatorValues.size() > 0) {
            ...
            totalInfoHeight += Math.max(param_font_metrics.getHeight(),
                value_font_metrics.getHeight()) * indicatorValues.size() +
                paramValueHeightMargin * (indicatorValues.size() - 1);
            index = 0;
            for (String indicatorParam : indicatorParams) {
                final String indicatorValue = indicatorValues.get(index++);
                final int paramStringWidth = param_font_metrics.stringWidth(indicatorParam + ":") +
                    paramValueWidthMargin + value_font_metrics.stringWidth(indicatorValue);
                if (maxInfoWidth < paramStringWidth)
                    maxInfoWidth = paramStringWidth;
            }
        }
        ...
        final int dateStringWidth = date_font_metrics.stringWidth(dateString);
        final int dateStringHeight = date_font_metrics.getHeight();
        ...
        final int maxStringWidth = Math.max(date_font_metrics.stringWidth(longDateString), Math.max(this.maxWidth, Math.max(dateStringWidth, maxInfoWidth)));
        ...
        if (_DatapadMode.equals(DatapadMode.FOLLOW_DOT)) {
            bestBorderX = suggestedBorderX > this.mainDrawArea.getX() ?
                    (suggestedBorderX + borderWidth) < (this.mainDrawArea.getX() + this.mainDrawArea.getWidth()) ? suggestedBorderX :
        ...
        int yy = (int)(y  + padding + date_font_metrics.getAscent() + 0.5);
        g2.setFont(date_font);
        g2.setColor(COLOR_BLUE);
        g2.drawString(dateString, (int) (((width - date_font_metrics.stringWidth(dateString)) >> 1) + x + 0.5), yy);
        index = 0;
        yy += date_font_metrics.getDescent() + dateInfoHeightMargin + value_font_metrics.getAscent();
        final String CLOSE_STR = Constants.COMPONENT_BUNDLE.getString("StockHistory_Close");
        for (String param : params) {
            final String value = values.get(index++);
            ...
            if (param.equals(CLOSE_STR)) {
                ...
                if (changePrice > 0.0)
                    g2.setColor(DEFAULT_HIGHER_NUMERICAL_VALUE_FOREGROUND_COLOR);
                else if (changePrice < 0.0)
                    g2.setColor(DEFAULT_LOWER_NUMERICAL_VALUE_FOREGROUND_COLOR);
            }
            g2.setFont(param_font);
            g2.drawString(param + ":", (int) (padding + x + 0.5), yy);
            g2.setFont(value_font);
            g2.drawString(value, (int) (width - padding - value_font_metrics.stringWidth(value) + x + 0.5), yy);
            // Same as yy += value_font_metrics.getDescent() + paramValueHeightMargin + value_font_metrics.getAscent()
            yy += paramValueHeightMargin + value_font_metrics.getHeight();
        }
        g2.setColor(Color.BLACK);
        ...
        for (String indicatorParam : indicatorParams) {
            ...
            g2.setFont(value_font);
            g2.drawString(indicatorValue, (int) (width - padding - value_font_metrics.stringWidth(indicatorValue) + x + 0.5), yy);
            // Same as yy += value_font_metrics.getDescent() + paramValueHeightMargin + value_font_metrics.getAscent()
            yy += paramValueHeightMargin + value_font_metrics.getHeight();
        }
        g2.setColor(oldColor);
    ...
    public void setDatapadMode(DatapadMode mode) { _DatapadMode = mode; setDirty(true); }
    ...
    protected void paintLayer(Graphics2D g2, JXLayer<? extends V> layer) {
        ...
        g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        g2.setColor(COLOR_BLUE);
        final int BALL_RADIUS = 8;//blue dot cursor
    ...
    private void updateIndicatorTraceInfos(int mainPointIndex) {
        ...
        final ChartPanel chartPanel = _pnlGraph.getChartPanel();
        Day day = null;
        final XYDataset xyDataset = _pnlGraph.getMainPlot().getDataset();
        if (xyDataset instanceof TimeSeriesCollection) {
            ...
            // 0 means main plot.
            final XYPlot plot = _pnlGraph.getMainPlot();
            ...
            final Date date = ((DefaultHighLowDataset)xyDataset).getXDate(0, mainPointIndex);
            ...
            // 0 means main plot.
            final XYPlot plot = this._pnlGraph.getMainPlot();
        ...
        for (int i = 1, size = _pnlGraph.getPlotSize(); i < size; i++) {
            final XYPlot plot = _pnlGraph.getPlot(i);
            final TimeSeriesCollection timeSeriesCollection = (TimeSeriesCollection)plot.getDataset();
    ...
    private TimeSeries getTimeSeries(int plotIndex, int seriesIndex) {
        ...
        if (plotIndex >= this._pnlGraph.getPlotSize()) {
            /* Not ready yet. */
            return null;
        }

        final XYDataset xyDataset = this._pnlGraph.getPlot(plotIndex).getDataset();
        if (xyDataset instanceof TimeSeriesCollection) {
            ...
            if (seriesIndex >= this._pnlGraph.getPlot(plotIndex).getDatasetCount()) {
                return null;
            }
            final XYDataset d = this._pnlGraph.getPlot(plotIndex).getDataset(seriesIndex);
    ...
    private Point2D.Double _getPointForCandlestick(int plotIndex, int seriesIndex, int dataIndex) {
        final ChartPanel chartPanel = this._pnlGraph.getChartPanel();
        final XYPlot plot = this._pnlGraph.getPlot(plotIndex);
    ...
    private boolean updateMainTraceInfo(Point2D point) {
        ...
        if (plot.getDataset() instanceof DefaultHighLowDataset)
            return _updateMainTraceInfoForCandlestick(point);

        // 0 are the main chart. 1, 2, 3... are TA.
        final TimeSeriesCollection all_series = (TimeSeriesCollection)plot.getDataset();
        if (all_series.getSeriesCount() == 0)
            return false;

        final TimeSeries timeSeries = all_series.getSeries(0);
        ...
        if (plotInfo.getSubplotCount() == 0)
            return false;
        PlotRenderingInfo subplotInfo = plotInfo.getSubplotInfo(0);
        if (subplotInfo == null)
            return false;

        final Rectangle2D _plotArea = subplotInfo.getDataArea();
        final ValueAxis domainAxis = plot.getDomainAxis();
        final RectangleEdge domainAxisEdge = plot.getDomainAxisEdge();
        final ValueAxis rangeAxis = plot.getRangeAxis();
        final RectangleEdge rangeAxisEdge = plot.getRangeAxisEdge();
