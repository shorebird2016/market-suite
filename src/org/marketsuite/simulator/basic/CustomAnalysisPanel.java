...
import org.marketsuite.framework.strategy.analysis.TransactionPanel;
 ...
 */
public class CustomAnalysisPanel extends AbstractStrategyPanel {
    public CustomAnalysisPanel() {
        ...
        west_pnl.add(new JLabel(FrameworkConstants.FRAMEWORK_BUNDLE.getString("custom_02")));
        ...
        _btnOpen.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent actionEvent) {//pick folder, replenish file list, analyze first symbol
                ...
                int ret = fc.showOpenDialog(CustomAnalysisPanel.this);
                if (ret == JFileChooser.APPROVE_OPTION) {
                    ...
                    try {
                        ArrayList<Transaction> log = _Engine.extractLog(
                            _sCurrentFile.getPath(), FrameworkConstants.FILE_FORMAT_CUSTOM);
                        if (log == null) {
                            MessageBox.messageBox(FrameworkConstants.FRAMEWORK_BUNDLE.getString("warning"),
...
//        cen_pnl.add(Box.createGlue());
        replaceTitleStrip(WidgetUtil.createTitleStrip(west_pnl, null, createCgarPanel()));

        //replace result table with trade table
        replaceResultPanel(_pnlTransaction = new TransactionPanel());
        _splHorizontal.setDividerLocation(500);
...
//        }
        ...
        //start analysis
        _Engine.setTransactionLog(_OrigTransactions);
...
//            plotPriceGraph();
     ...
     */
//    private TimeSeries annualReturnToTimeSeries(ArrayList<AnnualReturn> ars) {
...
//    private JComboBox<Integer> _cmbStartYear = new JComboBox<Integer>();
//    private JComboBox<Integer> _cmbEndYear = new JComboBox<Integer>();
