package org.marketsuite.framework.strategy.twosmacross;
...
*/
public class DualSmaCrossEngine extends AbstractEngine {
    //-----CTOR-----
    public DualSmaCrossEngine(FundData fund) { _Fund = fund; }
    ...
    public boolean simulate() throws IllegalArgumentException {
        ...
        if ( (start_index - end_index) < _nMa1Period)
        ...
        //retrieve MA period
        _nMa1Period = simParam.getMacOption().getEntryMA1();
        _nMa2Period = simParam.getMacOption().getEntryMA2();

        //if starting point does not have enough lookback, advance starting point to there is enough
        int len = _Fund.getQuote().size();
        if (len <= start_index + _nMa1Period) {
            int delta = _nMa1Period - (len - 1 - start_index);
            start_index = start_index - delta;
        }
        actualStartDate = _Fund.getDate(start_index);
        _MA1 = IndicatorUtil.calcSMA(_nMa1Period, len - _nMa2Period, _Fund);
        _MA2 = IndicatorUtil.calcSMA(_nMa2Period, len - _nMa2Period - 1, _Fund);
        _EntryCrossing = calcCrossing(_MA1, _MA2, len - _nMa2Period, end_index);
        _ExitCrossing = calcCrossing(_MA1, _MA2, len - _nMa2Period, end_index);
//        logCrossing(start_index, end_index);//for logging
        executeTrades(start_index, end_index);
//        logTransactions();//for logging
        return true;
    }
    public String getId() { return "Dual SMA Crossing"; }
     ...
     * inputs: _MA1, _EntryMA2, _ExitMA1, _ExitMA2; start_index = first data point to start comparison
...
//                                 && _MA1[loop_index] < _ExitMA2[loop_index]) {
//                            trade.sell(trade_date);
//                            _Transactions.add(new Transaction(trade));
//                            logTrade("Sell", trade_date, trade_price);
//                        }
...
//        System.out.println(buf);
    ...
    private float[] _MA1, _MA2; //use literal
    ...
    //----- literals -----
    public static int _nMa1Period = 50;
    public static int _nMa2Period = 200;
