...
public class SimUtil {
    ...
    public static double calcCagr(float num_years, float begin_value, float end_value) {
    ...
    public static void plotAnnualReturnGraph(AnnualReturnGraphPanel panel, SimReport sim_report) {
        ...
        TimeSeries sp = annualReturnToTimeSeries(FrameworkConstants.SP500_ANNUAL_RETURN, true);
        if (ars != null) {
            ts = annualReturnToTimeSeries(ars, false);
            ...
            sp = annualReturnToTimeSeries(sp_ars, true);
        }
...
if (FrameworkConstants.SP500_DATA.findQuoteByDate(eqty.getDate()) == null)
    System.err.println("------------");
     ...
     */
    public static TimeSeries annualReturnToTimeSeries(ArrayList<AnnualReturn> ars, boolean for_sp500) {
        String sym = ars.get(0).getSymbol();
        if (for_sp500) sym = "Buy/Hold SP500";
        TimeSeries ts = new TimeSeries(sym);
